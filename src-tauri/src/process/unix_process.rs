...
pub fn kill_process_group(pgid: u32) -> Result<(), String> {
    ...
    // 关键安全校验：在按进程组发送信号前，先确认该 pid 确实是「自己进程组的组长」
    // （即 pid == 真实 pgid）。若 process_group(0) 未生效（某些打包/启动环境下可能被
    // 静默忽略），子进程会继承父进程（本 app）的进程组，此时 kill -{pid} 可能命中
    // 一个并不以它为组长的进程组，进而误杀用户的其它无关软件（Linux deb 实测会导致
    // 多个软件同步崩溃）。校验失败则拒绝按组 kill，由上层回退到只杀直接 child 句柄。
    let actual_pgid = get_process_group_id(pgid)?;
    ensure_process_group_leader(pgid, actual_pgid)?;

    // 使用 "--" 分隔符，避免 "-{pgid}" 被 kill 当作选项解析。
...
/// 校验 pid 是否为「自己进程组的组长」（pid == 真实 pgid）。
/// 不匹配说明该进程未被隔离进独立进程组，按组 kill 会有误杀风险，拒绝执行。
#[cfg(any(unix, test))]
...
/// 解析 Linux /proc/<pid>/stat 中的 pgrp 字段。
/// comm 字段可能含空格/括号，故以最后一个 ')' 为界，其后第 3 个字段（pgrp）即进程组。
...
fn parse_linux_proc_stat_pgrp(stat: &str) -> Result<u32, String> {
    ...
    // "(comm)" 之后字段顺序为：state, ppid, pgrp, session, ...
...
/// 读取指定 pid 的真实进程组 ID（Linux：解析 /proc/<pid>/stat）。
#[cfg(target_os = "linux")]
...
/// 读取指定 pid 的真实进程组 ID（非 Linux 的 Unix，如 macOS：用 ps 查询 pgid）。
